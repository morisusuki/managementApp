	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (user != null && user.isEnabled() && userDAO.verifyPassword(username, password)) {
			...
			if ("admin".equals(user.getRole())) {
				...
				Map<String, Long> totalHoursByUser = attendanceDAO.findAll().stream().collect(Collectors.groupingBy(
						com.example.attendance.dto.Attendance::getUserId, Collectors.summingLong(att -> {
			...
			req.setAttribute("errorMessage",
					"ユーザーIDまたはパスワードが不正です。またはアカウントが無効です。");
